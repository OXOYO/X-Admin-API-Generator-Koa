...
const blogTagSchema = db.import('../../schema/blog_tag')
...
export default {
  getList: async (data, userInfo) => {
    ...
    let res = await blogTagSchema.findAndCountAll(options)
    return res
  },
  doAdd: async function (data) {
    let res = await blogTagSchema.findOrCreate({
      where: {
        name: data.name
      },
      defaults: data,
  ...
  doRemove: async function (data) {
    let res = await blogTagSchema.destroy({
      where: {
        id: data.id
      },
      logging: true
  ...
  doEdit: async function (data) {
    let res = await blogTagSchema.update(data, {
      where: {
        id: data.id
      },
      logging: true
