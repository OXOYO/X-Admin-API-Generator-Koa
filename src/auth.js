...
export default {
  // 生成token
  sign: function (data) {
    let token = jsonwebtoken.sign(data, CookieConfig.getItem('secret'), { expiresIn: '1d' })
    return token
  },
  // token 验证
  verifyToken: async function (ctx, next) {
    // 支持多种方式传递token
    let key = CookieConfig.getItem('token')
    ...
    if (token) {
      try {
        // 2.1.verify验证token
        decoded = jsonwebtoken.verify(token, CookieConfig.getItem('secret'))
